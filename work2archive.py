...
def make_rtf(sql_results):
	...
	table = paragraph.Table(PropertySets.TabPropertySet.DEFAULT_WIDTH*3, PropertySets.TabPropertySet.DEFAULT_WIDTH*7)
	# 

	edge = PropertySets.BorderPropertySet(width=20, style=PropertySets.BorderPropertySet.SINGLE)
	frame = PropertySets.FramePropertySet(edge, edge, edge, edge)

	for row in sql_results:
		c1 = paragraph.Cell(paragraph.Paragraph(ss.ParagraphStyles.Normal, str(row[0])), frame)
		row1 = ''
		if type(row[1]) is unicode:
			...
			row1 = ''
		print row1
		c2 = paragraph.Cell(paragraph.Paragraph(ss.ParagraphStyles.Normal, str(row1), frame))
		if row1!='':
			table.AddRow(c1, c2)
